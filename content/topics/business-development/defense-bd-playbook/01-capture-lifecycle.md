---
type: playbook
series: defense-bd-playbook
chapter: 1
title: The capture lifecycle
status: curated
tags: [business-development, defense, capture-management, govcon, proposals, playbook]
created: 2026-06-18
---

# Chapter 1 — The capture lifecycle

Capture is the disciplined process of turning a possible opportunity into a
winnable one, and then into a win. The model below follows the widely used
Shipley-style phased lifecycle, adapted for defense. Assessment: the exact phase
names vary by company, but the logic is industry-standard, and the single biggest
predictor of win rate is how early and how seriously you run the front end.

The timeline matters. For a major defense program, capture often starts 18 to 36
months before the RFP. The proposal phase, the part everyone notices, is the last
30 to 90 days, and by then your probability of winning is mostly already set.

## Phase 0 — Market and portfolio strategy

Before any single deal, decide where you are credible. Pick the customers,
mission areas, and contract vehicles where your past performance, clearances,
facilities, and people give you a real right to win. Assessment: a focused BD
portfolio beats an opportunistic one, because capture resources are scarce and
every pursuit you run is one you cannot run elsewhere. Output: a target list of
agencies, programs, and vehicles, and a deliberate list of things you will not
chase.

## Phase 1 — Opportunity identification

Build a pipeline of specific opportunities tied to that strategy. Sources include
SAM.gov pre-solicitation and sources-sought notices, agency long-range
acquisition forecasts, budget documents (the President's Budget and the
program-element justification books), industry-day announcements, market-research
firms (for example GovWin or Bloomberg Government), and your own customer
relationships. Each entry gets logged with an estimated value, expected RFP date,
incumbent, and contract vehicle. Detail is in [chapter 4](04-customer-stakeholders-shaping.md).

## Phase 2 — Opportunity assessment and qualification

Decide whether this one is worth real money. Run an early qualification and a
first probability-of-win (PWin) estimate, then take it through a gate review. Most
opportunities should die here, on purpose. The full method is in
[chapter 2](02-qualification-pwin-gates.md).

## Phase 3 — Capture planning (the core of capture)

Once an opportunity passes the qualification gate, assign a capture manager and
build the capture plan. This is where the win is actually engineered, over months,
not days. The capture plan pulls together:

- the customer and stakeholder map, and a call plan to engage each one
  (chapter 4),
- competitive analysis and a Black Hat assessment of the field (chapter 6),
- the teaming strategy and any teaming agreements (chapter 5),
- the solution concept and the discriminators that set you apart (chapter 6),
- the price-to-win analysis and target cost posture (chapter 6),
- the shaping actions: RFI responses, white papers, demos, and one-on-ones that
  legally influence the requirement before it is locked (chapter 4),
- a gap-closure plan with owners and dates for every weakness you found.

Assessment: the test of a capture plan is whether it changes what the customer
writes in the RFP and what they believe about you by the time it drops. A plan
that only prepares you to respond is not capture, it is just early proposal work.

## Phase 4 — Proposal planning

As the draft and final RFP approach, transition from capture to proposal. Stand up
the proposal team, build the compliance matrix from the Section L (instructions)
and Section M (evaluation factors) of the solicitation, lock the win themes and
solution, and schedule the color-team reviews. Capture intelligence hands off into
the proposal here. See [chapter 7](07-proposal-management.md).

## Phase 5 — Proposal development and submission

Write, review, and revise against the compliance matrix and the evaluation
criteria, running the color-team reviews (Pink, Red, Gold) along the way. Produce a
compliant, compelling, and clean proposal, then submit exactly per the
instructions. Compliance failures and missed deadlines are unforced losses.

## Phase 6 — Post-submission and award

After submission you may face evaluation notices, discussions, a request for a
final proposal revision (FPR), or oral presentations. Prepare for these as
deliberately as the written proposal. After award, always request a debrief, win
or lose. The debrief is your highest-value competitive intelligence and your
basis for deciding whether a bid protest is warranted (chapters 3 and 7).

## Why the front end wins

Assessment: companies that lose consistently almost always over-invest in
proposal heroics and under-invest in phases 0 through 3. The proposal can only
harvest the position that capture built. If you find yourself learning the
customer's hot buttons from the RFP, capture failed, and no amount of good writing
fully recovers it.

Next: [Qualification, PWin, and gates](02-qualification-pwin-gates.md).

## See also

- **In this series:** [Qualification, PWin, and gate reviews →](02-qualification-pwin-gates) · [Overview](../defense-bd-playbook)
