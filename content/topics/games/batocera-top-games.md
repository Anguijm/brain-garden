---
title: "Top 10 games on every system Batocera can run"
type: topic-note
category: games
tags: [emulation, batocera, games, retro, ps2, ps3, snes, n64, gamecube, wii, psp, 3ds, dreamcast, master-system, sega, sega-cd, 3do]
created: 2026-08-31
updated: 2026-08-31
sources_staged: false
draft: false
---

# Top 10 games on every system Batocera can run

Twenty-six systems, 260 games. Each list is ranked by a combination of critical consensus and lasting playability — not nostalgia. Scores are Assessment based on aggregated critical reception; nothing here was play-tested against this specific hardware. For PS3 and Xbox 360, compatibility with the emulator matters as much as game quality; notes on that are included.

---

## Navigation

| System | Era |
|---|---|
| [Sega Master System](#sega-master-system) | 1985–1996 |
| [Sega Game Gear](#sega-game-gear) | 1990–1997 |
| [NES](#nes) | 1983–1995 |
| [SNES](#snes) | 1990–2003 |
| [Sega Genesis](#sega-genesis-mega-drive) | 1988–1997 |
| [Sega CD / Mega CD](#sega-cd--mega-cd) | 1991–1996 |
| [TurboGrafx-16 / PC Engine](#turbografx-16--pc-engine) | 1987–1999 |
| [Neo Geo](#neo-geo) | 1990–2004 |
| [Sega Saturn](#sega-saturn) | 1994–2000 |
| [3DO](#3do) | 1993–1996 |
| [PlayStation (PS1)](#playstation-ps1) | 1994–2006 |
| [Nintendo 64](#nintendo-64) | 1996–2002 |
| [Dreamcast](#dreamcast) | 1998–2001 |
| [Game Boy](#game-boy) | 1989–2003 |
| [Game Boy Color](#game-boy-color) | 1998–2003 |
| [Game Boy Advance](#game-boy-advance) | 2001–2008 |
| [PlayStation 2](#playstation-2) | 2000–2013 |
| [GameCube](#gamecube) | 2001–2007 |
| [Nintendo DS](#nintendo-ds) | 2004–2014 |
| [PSP](#psp) | 2005–2014 |
| [Wii](#wii) | 2006–2013 |
| [PlayStation 3](#playstation-3) | 2006–2013 |
| [Xbox 360](#xbox-360) | 2005–2016 |
| [3DS](#3ds) | 2011–2020 |
| [Wii U](#wii-u) | 2012–2017 |
| [Arcade (MAME / FinalBurn Neo)](#arcade-mame--finalburn-neo) | All eras |

---

## Sega Master System

Sega's answer to the NES lost the North American market war but dominated Europe and Brazil, and its library reflects that: tighter ports of Sega's arcade output and a handful of RPGs that have no NES equivalent. Batocera runs it via Genesis Plus GX, the same core that handles the Genesis. Performance is perfect.

1. **Phantasy Star** (1987) — RPG — Assessment: 9/10
   One of the best RPGs of the 8-bit era, with 3D dungeons and a story that holds up. It is harder and stranger than early Final Fantasy, and it went places Nintendo-adjacent RPGs would not reach for years.

2. **Wonder Boy III: The Dragon's Trap** (1989) — Action-RPG — Assessment: 9/10
   A shape-shifting action RPG with a continuous world and transformation mechanics that feel modern. The 2017 remake exists if you want the facelift, but the original still plays cleanly.

3. **Alex Kidd in Miracle World** (1986) — Platformer — Assessment: 8/10
   Sega's mascot before Sonic. Built into many consoles from the factory. The boss fights are rock-paper-scissors, which is either charming or maddening depending on your tolerance. The later levels are harder than they look.

4. **R-Type** (1988) — Shoot-em-up — Assessment: 8.5/10
   The best port of the Irem arcade classic on a home console at the time. Slow, deliberate, and brutal — the charge shot and the Force pod system reward patience in a genre that usually rewards reflexes.

5. **Golden Axe Warrior** (1991) — Action-RPG — Assessment: 8/10
   A Zelda-style top-down RPG wearing Golden Axe clothes. Better than it has any right to be. If you played every Zelda and want something adjacent from the same era, this fills the gap.

6. **Shinobi** (1988) — Action — Assessment: 8/10
   A strong home conversion of the Sega arcade hit. Hostage rescue stages, throwing stars, and a bonus round that breaks the rhythm. Harder than the arcade version on later stages.

7. **Master of Darkness** (1992) — Action-platformer — Assessment: 8/10
   A Castlevania clone that is genuinely good. Victorian horror setting, whip-adjacent weapon mechanics, and a progression structure borrowed directly from Konami. Available in North America as Dracula (Game Gear version) but the Master System release is the complete one.

8. **Ys: The Vanished Omens** (1988) — Action-RPG — Assessment: 8/10
   The first entry in the Ys series, known for bump combat where you run into enemies at the right angle to deal damage. Odd but fast, and the soundtrack is one of the best on the hardware.

9. **Sonic the Hedgehog** (1991) — Platformer — Assessment: 7.5/10
   A scaled-back conversion of the Genesis launch title. Missing levels, reduced parallax, and the spin dash is absent in some versions. Still playable and historically significant, but play the Genesis version first.

10. **Psycho Fox** (1989) — Platformer — Assessment: 7.5/10
    A loose spiritual predecessor to the Wonder Boy series. Character selection changes your jump arc and a bird companion can be thrown at enemies. Underrated and rarely discussed outside of Master System enthusiasts.

---

## Sega Game Gear

Sega's handheld ran on the same hardware as the Master System, which means its library overlaps heavily and the ports are mostly faithful; Batocera runs it via the same Genesis Plus GX core, so everything here is solid. The battery life was infamous in 1990 but irrelevant in emulation.

1. **Sonic the Hedgehog** (1991) — Platformer — Assessment: 8.5/10
   A purpose-built Game Gear entry rather than a port, with its own level design and a smaller screen framing that actually suits the game. The best Sonic on the platform and one of the better handheld Sonic titles overall.

2. **Shining Force: The Sword of Hajya** (1993) — Tactical RPG — Assessment: 8.5/10
   A standalone entry in the Shining Force series scaled to handheld play. The strategy is as deep as the console originals and the story holds together independently. Rarely mentioned when people list the best Game Gear titles, and it should be.

3. **Defenders of Oasis** (1992) — RPG — Assessment: 8/10
   An Arabian Nights JRPG with a full party system, overworld exploration, and a story that takes itself seriously. The production values punch above the hardware. One of the best RPGs on any Sega handheld.

4. **Dragon Crystal** (1991) — Roguelike — Assessment: 7.5/10
   A dungeon crawler with permadeath and randomised floors. Short enough for handheld sessions and hard enough to keep you honest. A companion egg hatches into a creature that grows alongside your character.

5. **GG Shinobi** (1991) — Action — Assessment: 8/10
   Five ninjas, each with a distinct playstyle, and stages built around switching between them. Tight controls and a strong visual identity. Better than the Master System Shinobi games.

6. **Columns** (1990) — Puzzle — Assessment: 7.5/10
   Sega's answer to Tetris, launch title for the Game Gear. Match three gems of the same colour in any direction. Simpler than Tetris and faster to pick up; still playable thirty-five years later.

7. **Baku Baku Animal** (1996) — Puzzle — Assessment: 8/10
   Feed animals to matching food in a falling-block format. The two-player mode (link cable in original hardware, netplay via emulation) elevates it. One of the most distinctive puzzle games on the platform.

8. **Sonic Chaos** (1993) — Platformer — Assessment: 7.5/10
   The first Sonic with a dedicated run button and Tails as a playable character. Level design is looser than the original but the speed remains intact. Good for a second Sonic option after the first game.

9. **Crystal Warriors** (1991) — Tactical RPG — Assessment: 7.5/10
   A grid-based strategy RPG with elemental affinities and a small but functional party system. Predates Fire Emblem's western arrival and fills a similar niche for handheld play.

10. **Ristar** (1995) — Platformer — Assessment: 8/10
    A port of the underrated Genesis game. Star-shaped protagonist grabs enemies and pulls himself to surfaces. Satisfying physics-based mechanics and strong art direction. Arrived late in the platform's life and was overlooked accordingly.

---

## NES

The NES library has been picked over for forty years; what survives is genuinely hard and genuinely good. The UM790Pro runs everything at full speed with upscaling options that look better than they have any right to.

1. **Super Mario Bros. 3** (1988) — Platformer — Assessment: 10/10
   The high point of the NES era. Level design, power-up variety, and moment-to-moment feel have not aged. The only caveat is that you probably already know it.

2. **Mega Man 2** (1988) — Action-platformer — Assessment: 9.5/10
   The template every action-platformer copies. Eight robot masters, tight controls, and a difficulty curve that rewards learning rather than luck. The soundtrack is an outlier for the hardware.

3. **Metroid** (1986) — Action-adventure — Assessment: 9/10
   The original nonlinear exploration game. Atmospheric and disorienting in a way its successors have tried to recapture. Save states help with the brutal lack of in-game saving.

4. **Contra** (1988) — Run-and-gun — Assessment: 9/10
   Two-player co-op that still works. Hard enough that earning the ending feels meaningful. The Spread Gun remains one of the best weapon pickups in gaming.

5. **Castlevania** (1986) — Action-platformer — Assessment: 9/10
   Gothic atmosphere and punishing platforming. The whip-based combat aged better than most contemporaries. Symphony of the Night on PS1 is the better game, but this is where Simon Belmont was established.

6. **Tecmo Super Bowl** (1991) — Sports — Assessment: 8.5/10
   The best football game of its era, possibly the best arcade football game ever made. Barry Sanders is as broken as you remember.

7. **Mega Man 3** (1990) — Action-platformer — Assessment: 8.5/10
   More of Mega Man 2 with a slide mechanic and Proto Man cameos. Slightly easier, slightly longer, and sometimes underrated relative to its predecessor.

8. **DuckTales** (1989) — Platformer — Assessment: 8.5/10
   Capcom at the height of its Disney licensing era. Pogo-stick cane movement, non-linear world selection, and a Moon level theme that refuses to leave your memory.

9. **Battletoads** (1991) — Beat-em-up/Platformer — Assessment: 8/10
   Infamous difficulty makes it a genuine achievement to finish. The turbo tunnel section is a legitimate test. Best played with save states and no shame.

10. **Ninja Gaiden** (1988) — Action-platformer — Assessment: 8/10
    Cinematic story presentation by NES standards and tight action. The final stage's respawn mechanic will test patience. Worth it.

---

## SNES

The SNES library is one of the strongest in gaming history. Mode 7, the SPC700 sound chip, and a run of first- and third-party titles that defined genres still define genres. Everything runs perfectly.

1. **Super Metroid** (1994) — Action-adventure — Assessment: 10/10
   The blueprint for every Metroidvania. Atmosphere, pacing, and world design that have never been topped in the genre. Short by modern standards; replayable indefinitely.

2. **The Legend of Zelda: A Link to the Past** (1991) — Action-adventure — Assessment: 10/10
   The game that defined what a Zelda game is. Dual world mechanic, dungeon design, and overworld exploration that hold up without qualification.

3. **Chrono Trigger** (1995) — RPG — Assessment: 10/10
   The most efficient RPG ever made. Multiple endings, no random encounters after a point, a time travel story that pays off, and a combat system with dual and triple techs that rewards party experimentation.

4. **Super Mario World** (1990) — Platformer — Assessment: 10/10
   The launch title that remains one of the best games on the system. Yoshi, capes, secret exits, and a world map that rewards exploration. Bowser's castle sequence is a highlight of the era.

5. **Final Fantasy VI** (1994) — RPG — Assessment: 9.5/10
   Ensemble cast, opera scene, and Kefka — one of the only RPG villains who actually wins mid-game. The longest and most ambitious 16-bit JRPG.

6. **Donkey Kong Country 2: Diddy's Kong Quest** (1995) — Platformer — Assessment: 9.5/10
   Better than the original DKC in every respect. Pre-rendered graphics that still hold up, a pirate aesthetic, and Bramble Blast to haunt you.

7. **Street Fighter II Turbo** (1993) — Fighting — Assessment: 9/10
   The definitive home port of the arcade original for its time. Eight characters, tight controls, and the competitive depth that built the fighting game community.

8. **EarthBound** (1994) — RPG — Assessment: 9/10
   Satirical, strange, and emotionally effective. Combat is modern-day America via a Japanese lens, and the final sequence is unlike anything else on the platform.

9. **Super Castlevania IV** (1991) — Action-platformer — Assessment: 9/10
   The best Castlevania before Symphony of the Night redefined the series. Eight-directional whip, excellent level design, and a soundtrack that makes use of every SNES audio channel.

10. **Mega Man X** (1993) — Action-platformer — Assessment: 9/10
    The series reinvention. Dash mechanic, wall-climbing, and a difficulty curve that opens up as you collect abilities. The opening Sigma stage is a masterclass in teaching through play.

---

## Sega Genesis (Mega Drive)

The Genesis competed with the SNES on attitude and speed. Its library rewards players who want faster, harder, and louder. The UM790Pro's Vulkan renderer handles the hardware's quirky color palette well.

1. **Sonic the Hedgehog 2** (1992) — Platformer — Assessment: 9.5/10
   Speed-focused platforming at its peak for this hardware. Chemical Plant Zone, Mystic Cave, the Casino Night pinball sections. Tails co-op is optional and mostly harmless.

2. **Streets of Rage 2** (1992) — Beat-em-up — Assessment: 9.5/10
   The genre benchmark. Four characters with distinct movesets, a soundtrack by Yuzo Koshiro that holds up against anything, and two-player co-op that fills a room.

3. **Gunstar Heroes** (1993) — Run-and-gun — Assessment: 9.5/10
   Treasure's debut and still one of the finest action games on the platform. Weapon combining, bullet-hell boss fights, and a pace that never lets up.

4. **Castlevania: Bloodlines** (1994) — Action-platformer — Assessment: 9/10
   The only Castlevania on Genesis and one of its best entries. Two characters with different mechanics and a European Gothic aesthetic distinct from the SNES entries.

5. **Comix Zone** (1995) — Beat-em-up — Assessment: 8.5/10
   Unique comic-panel navigation mechanic and hand-drawn art that still looks good. Brutally short but brutally difficult. The soundtrack is exceptional for the hardware.

6. **Phantasy Star IV** (1993) — RPG — Assessment: 9/10
   The conclusion of the Phantasy Star saga. The best RPG on the Genesis, with a macro/combo system and a story that ties together threads from three previous games.

7. **Mortal Kombat II** (1994) — Fighting — Assessment: 8.5/10
   The Genesis version had blood enabled by default, making it the definitive home port over the censored SNES version. The fatality system was genuinely shocking for its time.

8. **Shinobi III: Return of the Ninja Master** (1993) — Action-platformer — Assessment: 8.5/10
   Fluid movement, multiple attack options, and boss fights that demand pattern recognition. The surfing and horseback levels are memorable setpieces.

9. **Road Rash II** (1992) — Racing — Assessment: 8/10
   Motorcycle racing with clubs and chains. Two-player split-screen, five countries, and a sense of speed that still reads. No racing game on this system is more fun in a room.

10. **ToeJam and Earl** (1991) — Roguelite — Assessment: 8/10
    Procedurally generated levels, two-player co-op, and a funk soundtrack. An odd game that found a cult following. The randomness means no two runs are identical.

---

## Sega CD / Mega CD

The Sega CD added CD-ROM storage to the Genesis, which the industry mostly used for FMV games that have not aged well — skip those entirely. What remains is a smaller set of titles that used the extra storage for actual game content: longer RPGs, better audio, and a handful of originals that never made it to cartridge. Batocera runs it via Genesis Plus GX alongside the Genesis library; if you have one set up you have the other automatically.

1. **Sonic CD** (1993) — Platformer — Assessment: 9/10
   The best Sonic game of the 16-bit era by most measures. Time travel mechanics, two possible futures per zone, and Amy Rose's first appearance. The Japanese/European soundtrack is the one to seek out; both versions are available in emulation without licensing barriers.

2. **Lunar: The Silver Star** (1992) — RPG — Assessment: 9/10
   A JRPG that took full advantage of CD audio with voiced cutscenes and an orchestrated score. Story, characters, and pacing are all stronger than contemporaries on cartridge. The Working Designs localisation is the version most people know.

3. **Lunar: Eternal Blue** (1994) — RPG — Assessment: 9/10
   The sequel is larger and more ambitious. Better dungeon design, more party members, and a story that pays off its predecessor's setup. The two Lunar games together represent the Sega CD's best extended argument for the format.

4. **Snatcher** (1994) — Adventure — Assessment: 9/10
   Hideo Kojima's cyberpunk visual novel, set in a future where synthetic humanoids are replacing and murdering humans. Dark, written with care, and the Western release is the only version with full English voice acting. A curiosity that rewards patience with the genre.

5. **Popful Mail** (1994) — Action-RPG — Assessment: 8.5/10
   An action-RPG with anime-style presentation, three playable characters with distinct mechanics, and a Working Designs localisation full of irreverent humour. Fast and playable in a way that most CD-era RPGs are not.

6. **Shining Force CD** (1994) — Tactical RPG — Assessment: 8.5/10
   A compilation of the two Game Gear Shining Force games with enhanced visuals and a new arena mode. The tactical RPG mechanics hold up and the CD format allowed for full chapter saves that the handheld originals lacked.

7. **Final Fight CD** (1993) — Beat-em-up — Assessment: 8/10
   The definitive home version of the Capcom arcade game. Three playable characters, two-player co-op, and none of the cuts that hobbled the SNES port. The soundtrack benefits noticeably from CD audio.

8. **Silpheed** (1993) — Shoot-em-up — Assessment: 8/10
   A vertical shooter that used CD-rendered pre-rendered 3D backgrounds scrolling behind sprite-based gameplay. The visual approach was striking for 1993 and the shooting is solid underneath it. More show than depth but the show holds up.

9. **Batman Returns** (1993) — Action — Assessment: 7.5/10
   Konami's beat-em-up adaptation, with a style and tone closer to the film than most licensed games manage. Better than the SNES version; one of the better Batman games of the era.

10. **Night Trap** (1992) — FMV — Assessment: 6/10
    Historically significant as the FMV game that triggered the 1993 Senate hearings that led to the ESRB rating system. The gameplay is simple to the point of being mechanical. Worth playing once to understand why it caused the reaction it did; not worth playing twice.

---

## TurboGrafx-16 / PC Engine

NEC's console sold modestly in the West but dominated in Japan under the PC Engine name, and its CD-ROM library — available in Batocera via Mednafen's PCE core — contains some of the finest RPGs and shoot-em-ups of the era. The core handles both the base cartridge library and the CD add-on with no configuration needed.

1. **Castlevania: Rondo of Blood** (1993) — Action-platformer — Assessment: 9.5/10
   Long considered the pinnacle of classic Castlevania, exclusive to the PC Engine CD until its Wii Virtual Console release. Multiple branching paths, a hidden playable character, and the tightest control in the series before Symphony of the Night. The version that Dracula X on SNES was a watered-down port of.

2. **Ys Book I and II** (1989) — Action-RPG — Assessment: 9/10
   The CD format gave Ys a fully voiced cast and a redbook audio soundtrack that was revelatory in 1989. Bump combat, a continuous two-game story, and production values that embarrassed contemporary console RPGs. The gateway drug for the entire Ys franchise.

3. **Blazing Lazers** (1989) — Shoot-em-up — Assessment: 9/10
   A vertical shmup that pushed the hardware on launch and still looks strong. Weapon upgrades stack into combinations and the bosses scale well across the game's difficulty curve. One of the best shooters of the era, period.

4. **Bonk's Adventure** (1990) — Platformer — Assessment: 8.5/10
   The TurboGrafx mascot game. Head-butt enemies, spin off ledges, and eat meat to power up. Charming and physically inventive in ways that feel distinct from Nintendo platformers of the same period.

5. **Military Madness** (1989) — Tactical strategy — Assessment: 8.5/10
   A hex-grid turn-based strategy game with unit variety and terrain effects. The AI is competent enough to teach you the mechanics. One of the first strategy games on a home console that felt designed for adults.

6. **Gate of Thunder** (1992) — Shoot-em-up — Assessment: 8.5/10
   A CD-exclusive horizontal shmup with a hard rock soundtrack and cinematic presentation. The weapon system and stage design hold it above most contemporaries. Often bundled with Bonk's Adventure on the CD add-on.

7. **Lords of Thunder** (1993) — Shoot-em-up — Assessment: 8.5/10
   Spiritual successor to Gate of Thunder with selectable stage order and elemental armour upgrades. The soundtrack is arguably better. Both games belong on any shmup shortlist.

8. **Dungeon Explorer** (1989) — Action-RPG — Assessment: 8/10
   A top-down action RPG with five-player multiplayer. Hack through dungeons, collect items, level up. The multiplayer is the main draw; solo play is functional but less engaging. A landmark for cooperative console gaming.

9. **Neutopia** (1990) — Action-adventure — Assessment: 8/10
   The TurboGrafx Zelda. Not a clone — clearly inspired but with its own dungeon logic and overworld structure. If you exhausted A Link to the Past and want more of the same design philosophy, this fills the gap.

10. **Magical Chase** (1991) — Shoot-em-up — Assessment: 8/10
    A horizontally scrolling shmup with a witch protagonist and a deceleration mechanic that makes positioning deliberate. Expensive on the original hardware market; emulation is the practical way to play it. Visually distinctive and mechanically sound.

---

## Neo Geo

SNK's home console was priced for arcades — the cartridges cost as much as a console from another manufacturer — which means its library is small and focused almost entirely on fighting games and shoot-em-ups. Batocera runs it via FinalBurn Neo with near-perfect accuracy. Everything here ran in SNK's own arcades before it came home.

1. **Metal Slug** (1996) — Run-and-gun — Assessment: 9.5/10
   The definitive run-and-gun. Hand-drawn animation that has not aged, tight controls, and a difficulty that rewards repetition without feeling unfair. Every sequel is worth playing; this is where to start.

2. **Metal Slug 3** (2000) — Run-and-gun — Assessment: 9.5/10
   The high point of the series. Branching paths, the longest and strangest final stage in the franchise, and a level of animation detail that still impresses. The slug variety peaks here.

3. **Garou: Mark of the Wolves** (1999) — Fighting — Assessment: 9.5/10
   SNK's best fighting game. The T.O.P. system, Just Defend mechanic, and a roster built from scratch give it a competitive depth that holds up in 2026. The Street Fighter IV of SNK games.

4. **The King of Fighters '98** (1998) — Fighting — Assessment: 9/10
   The consensus peak of the KOF series. No story, just a tournament — which means SNK spent the entire budget on balance and roster. Still played competitively. The best team-based fighter on the hardware.

5. **Samurai Shodown II** (1994) — Fighting — Assessment: 9/10
   Weapon-based fighting with a pace slower than Street Fighter and a damage model where a single hit changes the match. Rage mode, disarms, and a roster with genuine personality. The competitive standard for the series.

6. **Pulstar** (1995) — Shoot-em-up — Assessment: 8.5/10
   A horizontal shmup clearly inspired by R-Type but with its own option system and a Neo Geo hardware showcase in every stage. Harder than it looks and visually unlike anything on other platforms.

7. **Blazing Star** (1998) — Shoot-em-up — Assessment: 8.5/10
   A spiritual successor to Pulstar with a combo-scoring system and seven ships with distinct behaviours. The bonus scoring mechanics add a layer beyond survival. One of the finest shmups of the late 90s.

8. **Last Blade 2** (1998) — Fighting — Assessment: 8.5/10
   Weapon-based fighting with a Speed/Power mode split that changes your playstyle from the start screen. The animation is some of the best sprite work SNK produced. A fighting game that rewards patience.

9. **Neo Turf Masters** (1996) — Sports — Assessment: 8/10
   The best arcade golf game ever made. Six courses, simple controls, and an addictive shot-shaping mechanic. Not a simulation — a score-chasing game with perfect feedback loops. Often overlooked in discussions of the Neo Geo library.

10. **Shock Troopers** (1997) — Run-and-gun — Assessment: 8/10
    A top-down run-and-gun with three-soldier team selection and multiple routes through each level. Fast, loud, and well-designed. Closer to Ikari Warriors than Metal Slug but holds its own.

---

## Sega Saturn

The Saturn's library is one of gaming's best-kept secrets — strong 2D fighters, RPGs, and exclusives that never made it to other platforms. The UM790Pro handles it well via Mednafen/Beetle Saturn.

1. **Panzer Dragoon Saga** (1998) — RPG — Assessment: 10/10
   One of the rarest and most expensive physical games in existence. The RPG branch of the Panzer Dragoon series, with a unique 3D flight combat system and a story worth the trip. Physical copies sell for \$600+; this is the emulation use case.

2. **Radiant Silvergun** (1998) — Shoot-em-up — Assessment: 10/10
   The argument for the shoot-em-up as an art form. Seven weapons with real strategic depth, a story delivered between stages, and a scoring system worth mastering.

3. **Guardian Heroes** (1996) — Beat-em-up/RPG — Assessment: 9.5/10
   Branching narrative beat-em-up with an RPG leveling system and up to six simultaneous players. Treasure again. The branching system means replayability is built in.

4. **Nights into Dreams** (1996) — Action — Assessment: 9/10
   Sega's attempt to define 3D for the Saturn. Flight-based movement through dream worlds, a score system tied to performance, and a visual style that holds up. The Christmas NiGHTS bonus disc is also worth loading.

5. **Castlevania: Symphony of the Night** (1997) — Action-RPG — Assessment: 10/10
   Available on PS1 as well, but the Saturn version includes additional content. The game that turned Castlevania into a Metroidvania and defined the subgenre's conventions.

6. **Burning Rangers** (1998) — Action — Assessment: 8.5/10
   Sega's last major Saturn release. Firefighting third-person action in fully 3D environments, by the Sonic Team. Technically impressive for the hardware and narratively earnest.

7. **Dragon Force** (1996) — Strategy/RPG — Assessment: 9/10
   Large-scale tactical RPG with battles involving hundreds of units. Eight monarchs to choose from, each with different story paths. One of the most ambitious games the Saturn produced.

8. **Sega Rally Championship** (1995) — Racing — Assessment: 8.5/10
   The best racing game on the platform. Three courses, physics that model surface conditions, and a feel that the Saturn's hardware suited better than it suited most 3D games.

9. **Street Fighter Alpha 2** (1996) — Fighting — Assessment: 9/10
   The Saturn version is the closest home port to the CPS2 arcade original. Custom combos, a large roster, and the peak of the Alpha series.

10. **Shining Force III** (1998) — Tactical RPG — Assessment: 9/10
    Three interconnected scenarios released in Japan; only Scenario 1 was released in the West. The Saturn's best tactical RPG and a strong one by any standard. Fan translations cover the other scenarios.

---

## 3DO

The 3DO launched in 1993 at \$699 — three times the price of a Genesis — and never recovered from it. Its library is thin and uneven, but the hardware was genuinely capable and a handful of titles made use of it. Batocera runs 3DO via the Opera core; compatibility is reasonable for the titles that matter. The console's main claim to relevance today is as the origin platform for a few franchises and ports that were exceptional for their time.

1. **Road Rash** (1994) — Racing — Assessment: 8.5/10
   The definitive version of EA's motorcycle combat racing series. The 3DO release added CD audio (licensed rock tracks), full motion video cutscenes between races, and a much larger world than earlier versions. The brawling and racing hold up better than most 3DO titles.

2. **Star Control II: The Ur-Quan Masters** (1994) — Adventure/Strategy — Assessment: 9/10
   A vast open-ended space adventure with diplomacy, resource gathering, and real-time ship combat. The 3DO port added full voice acting to what was already one of the deepest science fiction games ever made. The open-source remake (The Ur-Quan Masters) is free, but the 3DO version is worth experiencing for the presentation.

3. **Super Street Fighter II Turbo** (1994) — Fighting — Assessment: 8.5/10
   The most complete and accurate home port of Capcom's fighter at the time of release. All characters, all modes, and the closest to arcade timing of any home version in 1994. The 3DO's processing power showed here in a way it rarely did elsewhere.

4. **Gex** (1995) — Platformer — Assessment: 7.5/10
   A mascot platformer that was technically impressive for 1994 and went on to spawn a franchise. The gecko protagonist clings to walls, the level themes are TV-channel parodies, and the pop-culture one-liners were the main selling point. Holds up as a competent platformer once the jokes expire.

5. **Alone in the Dark** (1994) — Survival horror — Assessment: 8/10
   The game that established the survival horror genre before Resident Evil named it. Pre-rendered backgrounds, tank controls, and limited resources. The 3DO port is the best pre-PS1 version of a historically important game.

6. **D** (1995) — FMV/Adventure — Assessment: 7.5/10
   A horror adventure from Kenji Eno with almost no gameplay — you walk through environments and watch the story unfold. Disturbing subject matter, a two-hour real-time limit, and a tone unlike anything else from the era. An artefact more than a game, but a genuinely unsettling one.

7. **Killing Time** (1995) — First-person shooter — Assessment: 7.5/10
   A first-person shooter set in a 1930s mansion with FMV-based enemies. The enemy variety and atmosphere separate it from contemporaries. Controls poorly by modern standards but the setting is more interesting than most shooters of the period.

8. **PO'ed** (1995) — First-person shooter — Assessment: 7/10
   A fast, chaotic shooter with a jetpack mechanic that allowed full three-dimensional movement — unusual for 1995. More technically interesting than narratively coherent, but the freedom of movement was ahead of its time.

9. **Slayer** (1994) — RPG — Assessment: 7/10
   A dungeon RPG with first-person exploration and a party system. Unremarkable by the standards of the genre but one of the few 3DO titles that committed to depth over spectacle. Works better in emulation where load times are reduced.

10. **Total Eclipse** (1994) — Rail shooter — Assessment: 7/10
    A 3D rail shooter with pre-rendered environments and a solar eclipse as its central visual motif. The spectacle was the point in 1994 and the shooting underneath it is functional. Representative of what the 3DO was trying to be at launch.

---

## PlayStation (PS1)

The PS1 library defined the 32-bit era and delivered the first wave of 3D gaming done well. The UM790Pro handles it at full speed with upscaling that smooths the low-resolution textures without destroying the aesthetic.

1. **Final Fantasy VII** (1997) — RPG — Assessment: 9.5/10
   The game that brought JRPGs to a mainstream Western audience. Materia system, Midgar, the moment in Disc 1 that everyone remembers. Pre-rendered backgrounds hide the polygon limitations gracefully.

2. **Castlevania: Symphony of the Night** (1997) — Action-RPG — Assessment: 10/10
   The best action-RPG on the platform and the game that named a subgenre. Inverted castle, equipment system, and more content than most games twice its size.

3. **Metal Gear Solid** (1998) — Stealth-action — Assessment: 9.5/10
   Cinematic storytelling, stealth mechanics that still feel considered, and boss fights designed around the controller. The codec calls alone justify the playthrough.

4. **Resident Evil 2** (1998) — Survival horror — Assessment: 9.5/10
   Two interconnected campaigns, limited resources, and a tension level that the fixed camera angles enhanced rather than limited. Leon and Claire's stories change based on order of play.

5. **Tekken 3** (1998) — Fighting — Assessment: 9.5/10
   The best 3D fighting game of the era. Fast, deep, and filled with characters that have remained in the series for decades. Tekken Force mode is a bonus.

6. **Tony Hawk's Pro Skater 2** (2000) — Sports/Action — Assessment: 9.5/10
   The high point of the THPS formula. Create-a-skater, level editor, and a goal structure that rewards mastery. The control scheme remains the template.

7. **Silent Hill** (1999) — Survival horror — Assessment: 9/10
   Psychological horror through fog and static. The town of Silent Hill as a manifestation of trauma is a more interesting concept than Resident Evil's monster houses, and it mostly works.

8. **Spyro the Dragon** (1998) — 3D platformer — Assessment: 8.5/10
   The best 3D platformer on the platform that isn't Crash Bandicoot. Collectible-driven exploration, flight sections, and a color palette that still reads well.

9. **Crash Bandicoot 2: Cortex Strikes Back** (1997) — 3D platformer — Assessment: 9/10
   The series peak. Tighter level design than the original, a wider move set, and a difficulty curve that peaks with the bonus levels. Harder than it looks.

10. **Chrono Cross** (1999) — RPG — Assessment: 9/10
    Controversial as a Chrono Trigger successor because it barely connects to the original. On its own terms: enormous cast, elemental combat system, and a soundtrack by Yasunori Mitsuda that is among the best in the medium.

---

## Nintendo 64

The N64 library is smaller than its contemporaries but contains some of the most influential 3D games ever made. Dolphin handles N64 emulation through the Wii's Virtual Console versions for many titles; standalone N64 emulation via Mupen64Plus is also solid.

1. **The Legend of Zelda: Ocarina of Time** (1998) — Action-adventure — Assessment: 10/10
   The game that defined 3D action-adventure. Z-targeting, context-sensitive actions, and a world that taught players how to navigate 3D space. Its influence is still visible everywhere.

2. **Super Mario 64** (1996) — 3D platformer — Assessment: 10/10
   The template for 3D platformers. 120 stars, a hub world that rewards exploration, and movement mechanics — wall kicks, long jumps, triple jumps — that speedrunners still optimize.

3. **GoldenEye 007** (1997) — First-person shooter — Assessment: 9.5/10
   The console FPS before Halo. Objective-based missions, stealth options, and four-player split-screen deathmatch that defined couch multiplayer for years. Controls age poorly without modern camera inversion.

4. **The Legend of Zelda: Majora's Mask** (2000) — Action-adventure — Assessment: 9.5/10
   A darker, stranger Zelda built on a three-day loop mechanic. Fewer dungeons than Ocarina, more character depth, and a structure that rewards mastery of the clock system.

5. **Super Smash Bros.** (1999) — Fighting — Assessment: 9/10
   The original four-player party fighter. Simpler than its successors, faster in some respects, and still fun with the right group. Pikachu remains the pick of competitive players from this era.

6. **Banjo-Kazooie** (1998) — 3D platformer — Assessment: 9.5/10
   Rare's best. Collectible-hunting through imaginatively designed worlds, excellent note/jiggy economy, and a duo with genuine chemistry. Tooie is larger but more bloated.

7. **Donkey Kong 64** (1999) — 3D platformer — Assessment: 8/10
   Massive and collectible-heavy to a fault — five characters each with their own items is 20% more than needed. The execution is solid and the multiplayer mode holds up, but it outstays its welcome.

8. **Star Fox 64** (1997) — Rail shooter — Assessment: 9/10
   Branching path system, fully voiced cast, and a campaign length that suits the genre. "Do a barrel roll" is a meme; the game deserves better than the meme.

9. **Paper Mario** (2001) — RPG — Assessment: 9/10
   The RPG that made Mario's world feel inhabited. Turn-based combat with timing-based action commands, a chapter structure, and writing sharper than any Mario game before it.

10. **F-Zero X** (1998) — Racing — Assessment: 9/10
    Thirty cars at extreme speed with a death mechanic that punishes mistakes honestly. No modern racing game captures this specific kind of speed. The randomized track generator adds replayability.

---

## Dreamcast

The Dreamcast had an 18-month window of being the most powerful consumer hardware available, and Sega filled it with experiments. Many of its best games never appeared elsewhere. Flycast handles Dreamcast emulation well on Batocera.

1. **Shenmue** (1999) — Action-adventure — Assessment: 9/10
   Open world, real-time clock, and a scope that no game had attempted before. Slow by modern standards in the best way. The second chapter follows and requires this one.

2. **Jet Grind Radio** (2000) — Action/Sports — Assessment: 9/10
   Cell-shaded graffiti and inline skates. A visual identity that holds up completely and a soundtrack that defines the era's sound. Short, replayable, and unlike anything else.

3. **Soul Calibur** (1999) — Fighting — Assessment: 9.5/10
   The arcade port that exceeded its source. Eight-way movement, a mission mode, and a roster with distinct weapons-based styles. The console port was technically superior to the arcade version at launch.

4. **Resident Evil: Code Veronica** (2000) — Survival horror — Assessment: 8.5/10
   The largest and most ambitious classic-era Resident Evil. Fixed cameras, tank controls, and a story that bridges RE2 and RE4 in scope.

5. **Phantasy Star Online** (2000) — Action-RPG — Assessment: 9/10
   The console MMO before consoles had reliable internet. Offline play is viable and still enjoyable; the social design assumptions are visible even single-player. Private servers restore the online experience.

6. **Skies of Arcadia** (2000) — RPG — Assessment: 9/10
   Airship exploration RPG with a high encounter rate that is the one genuine flaw. Otherwise: excellent world, memorable party, and sky-pirate adventure that few RPGs have matched for tone.

7. **Marvel vs. Capcom 2** (2000) — Fighting — Assessment: 9/10
   56 characters, three-on-three combat, and an assist system that enabled combinations never anticipated by the designers. The competitive scene it spawned lasted decades.

8. **Crazy Taxi** (2000) — Arcade — Assessment: 8.5/10
   Get passengers to their destinations before time runs out. Simple, loud, and instantly understandable. The Offspring and Bad Religion soundtrack cannot be separated from the experience.

9. **Power Stone 2** (2000) — Fighting — Assessment: 8.5/10
   Four-player arena fighter with dynamic stages that collapse, transform, and supply weapons. The best multiplayer game on the platform.

10. **Ikaruga** (2001) — Shoot-em-up — Assessment: 9.5/10
    Polarity-switching mechanic turns bullet avoidance into puzzle solving. Brief, brutal, and designed around a scoring system that rewards pattern mastery. One of the finest shoot-em-ups ever made.

---

## Game Boy

Nintendo's original handheld ran for over a decade on four AA batteries and a green-tinted screen, which is why its library is enormous and its best games were designed around constraints that made them timeless. Batocera runs it via mGBA, the most accurate GB/GBC/GBA emulator available, at full speed with optional colour correction and integer scaling.

1. **Tetris** (1989) — Puzzle — Assessment: 10/10
   The definitive version of the definitive puzzle game. The Game Boy port's simplicity — no frills, no modes, just Tetris — is the point. The bundled game on launch hardware in most regions; it sold the system and the concept simultaneously.

2. **Pokemon Red / Blue** (1996) — RPG — Assessment: 9/10
   The games that defined portable RPGs. 151 creatures, a trading mechanic that required a second player, and a world with enough secrets to support a decade of schoolyard rumour. Mechanically simple by later standards; historically irreplaceable.

3. **The Legend of Zelda: Link's Awakening** (1993) — Action-adventure — Assessment: 9.5/10
   A full Zelda on a handheld, with a stranger story than any console entry before it and a dreamlike coastal island that holds together as a world. The DX colour version on GBC adds a bonus dungeon and is the preferred version.

4. **Kirby's Dream Land** (1992) — Platformer — Assessment: 8.5/10
   Kirby's debut. Short, breezy, and mechanically clean. The copy ability is not yet present — Kirby only inhales and spits — which makes this a purer expression of the character's movement feel. Beatable in an afternoon; worth it.

5. **Metroid II: Return of Samus** (1991) — Action-adventure — Assessment: 8.5/10
   The bridge between the NES original and Super Metroid. Dark, claustrophobic, and mechanically inventive within the hardware's limits. The mission structure — hunt a fixed number of Metroids — gives it a shape the other games lack. The 3DS remake exists; the original still holds up independently.

6. **Donkey Kong** (1994) — Platformer/Puzzle — Assessment: 9/10
   Not just a port of the arcade game. The first four stages are the arcade levels; then it opens into a full puzzle-platformer with 100-plus stages across nine worlds. DK gains a full moveset. One of the best games on the hardware by a significant margin.

7. **Mega Man V** (1994) — Action-platformer — Assessment: 9/10
   The only Game Boy Mega Man with an original robot master roster (the Stardroids) rather than recycled NES bosses. New weapons, new mechanics, and production values that push the hardware. The peak of the Game Boy Mega Man series.

8. **Final Fantasy Adventure** (1991) — Action-RPG — Assessment: 8.5/10
   The game that became the Mana series. Top-down action combat, a world map, and a story with more weight than most 8-bit RPGs attempt. The GBA remake (Sword of Mana) exists if you want the visual upgrade; the original plays faster.

9. **Pokemon Gold / Silver** (1999) — RPG — Assessment: 9.5/10
   Technically Game Boy Color but backward-compatible with the original hardware. Two full regions, a day-night cycle, and 251 Pokemon. The most content ever packed into a Game Boy cartridge. Crystal is the definitive version.

10. **Gargoyle's Quest** (1990) — Action-platformer — Assessment: 8/10
    A Ghosts 'n Goblins spinoff where you play the gargoyle villain. Side-scrolling action alternates with an overworld and RPG progression. Harder than it looks and more mechanically varied than most GB action games.

---

## Game Boy Color

The GBC sits in an odd position — backward-compatible with the original Game Boy, forward-overshadowed by the GBA — but its exclusive library contains some of the most polished handheld games before the GBA era. Batocera's mGBA core handles it with full accuracy and proper colour rendering.

1. **The Legend of Zelda: Oracle of Ages / Oracle of Seasons** (2001) — Action-adventure — Assessment: 9/10
   Two linked games developed simultaneously by Capcom under Nintendo's direction. Each is a full Zelda; completing both and linking the saves unlocks a combined true ending. Ages focuses on puzzles, Seasons on combat. Play them in any order.

2. **Metal Gear Solid** (2000) — Stealth-action — Assessment: 8.5/10
   A purpose-built GBC entry that is a genuine Metal Gear game, not a port. Overhead perspective, stealth mechanics, codec conversations, and a story that connects to the PlayStation original. Remarkable for the hardware.

3. **Dragon Warrior Monsters** (1998) — RPG — Assessment: 8.5/10
   The game Pokemon players wanted more of: monster collection with a breeding system that generates new creatures from combinations. Deeper than Pokemon at the time in terms of team construction options.

4. **Shantae** (2002) — Platformer — Assessment: 9/10
   Released at the very end of the GBC's life, which is why it sold poorly and now costs hundreds of dollars on original cartridge. A metroidvania with belly-dance transformation mechanics, expressive sprite animation, and a world that rewards exploration. The definitive late-era GBC game.

5. **Pokemon Crystal** (2000) — RPG — Assessment: 9/10
   The enhanced version of Gold/Silver with animated Pokemon sprites, a more developed story, and the Battle Tower endgame. The best Pokemon game on the original hardware family.

6. **Wario Land 3** (2000) — Platformer — Assessment: 8.5/10
   Wario's platformers inverted the genre conventions — you cannot die, but you need to find four treasures per level across multiple visits as new abilities unlock new routes. Clever, unhurried, and mechanically satisfying.

7. **Bionic Commando: Elite Forces** (2000) — Action — Assessment: 8/10
   A GBC original that captures the grappling-hook mechanics of the NES classic in a new mission structure. Plays better than most GBC action titles and was developed by Nintendo Software Technology.

8. **Harry Potter and the Chamber of Secrets** (2002) — Action-RPG — Assessment: 7.5/10
   The GBC version is a top-down RPG entirely distinct from the console releases. Well-designed for the hardware, with spell-based combat and a Hogwarts that feels navigable. Better than its licensed-game reputation suggests.

9. **Resident Evil Gaiden** (2001) — Action-RPG — Assessment: 7/10
   An odd entry in the series — a top-down RPG with a first-person shooting minigame for combat. Not canon; not the best Resident Evil. Worth playing as a curiosity and better than the reviews from 2001 suggest.

10. **Alone in the Dark: The New Nightmare** (2001) — Action-adventure — Assessment: 7.5/10
    A top-down version of the PS1 survival horror game. The GBC version tells the same story from a different angle with original gameplay. One of the most technically accomplished GBC titles and largely forgotten.

---

## Game Boy Advance

The GBA library is deep with RPGs, platformers, and ports that occasionally exceeded their console originals. mGBA on Batocera is cycle-accurate and handles the full library without issue.

1. **Mother 3** (2006) — RPG — Assessment: 10/10
   Never officially released in English; the fan translation by Clyde Mandelin is the definitive version. Emotional, strange, and willing to go places most RPGs avoid. The battle system uses rhythm.

2. **Final Fantasy VI Advance** (2006) — RPG — Assessment: 9.5/10
   The best version of FF6 at time of release, with added content and a re-translated script. The sound chip is weaker than the SNES original but the added dungeon and espers are worth it.

3. **Golden Sun** (2001) — RPG — Assessment: 9/10
   The GBA's flagship RPG. Djinn system, Psynergy puzzle mechanics, and a story that demanded its sequel. Technically impressive for the hardware.

4. **Metroid Fusion** (2002) — Action-adventure — Assessment: 9.5/10
   Linear by Metroid standards, tense by any standard. The SA-X pursuit sequences are among the most effectively frightening moments in the series.

5. **Castlevania: Aria of Sorrow** (2003) — Action-RPG — Assessment: 9.5/10
   The peak of the GBA Castlevania trilogy. Soul Absorb mechanic adds RPG depth to the Metroidvania structure. Better paced than its predecessors.

6. **Fire Emblem: The Sacred Stones** (2004) — Tactical RPG — Assessment: 9/10
   The most accessible Fire Emblem released in English before the series became mainstream. Two routes, a world map for grinding, and permadeath that makes every decision matter.

7. **Advance Wars** (2001) — Tactical strategy — Assessment: 9.5/10
   The turn-based strategy game that introduced the genre to a generation. CO powers, terrain advantages, and a campaign long enough to teach every mechanic before testing it.

8. **The Legend of Zelda: The Minish Cap** (2004) — Action-adventure — Assessment: 9/10
   Capcom-developed Zelda with a size-shifting mechanic. Compact, polished, and underplayed relative to its quality.

9. **Pokémon Emerald** (2004) — RPG — Assessment: 9/10
   The definitive Generation III Pokémon game. Battle Frontier post-game extends playtime significantly. The Hoenn region is divisive; the execution is not.

10. **Tactics Ogre: The Knight of Lodis** (2001) — Tactical RPG — Assessment: 8.5/10
    A prequel to Tactics Ogre on SNES. Grid-based combat with a branching story driven by alignment choices. Deeper than it appears at first.

---

## PlayStation 2

The best-selling console ever produced has a library that justifies the claim. PCSX2 on Batocera runs the PS2 library with Vulkan rendering and handles upscaling cleanly. Enable the Vulkan renderer.

1. **Shadow of the Colossus** (2005) — Action-adventure — Assessment: 10/10
   Sixteen boss fights, no other enemies, and a desolate world that makes each colossus feel like an event. The moral weight accumulates. One of the clearest arguments for games as art.

2. **Metal Gear Solid 3: Snake Eater** (2004) — Stealth-action — Assessment: 10/10
   The Cold War prequel. Camouflage system, survival mechanics, and a final hour that ranks among the best storytelling in gaming. The Subsistence version adds online modes and a third-person camera option.

3. **God of War II** (2007) — Action — Assessment: 9.5/10
   The technical ceiling of the PS2. Scale that the hardware had no right to produce, combat that improved on the original in every way, and setpieces that run continuously.

4. **Final Fantasy XII** (2006) — RPG — Assessment: 9/10
   Gambit system lets you program party AI; exploration is open from early in the game. Divisive at launch for moving away from JRPG conventions; reappraised as the best FF since VI.

5. **Ico** (2001) — Action-adventure — Assessment: 9.5/10
   Shadow of the Colossus's companion piece. A boy leading a girl through a castle, with minimal combat and environmental puzzle design. The hand-holding mechanic is literal and deliberate.

6. **Okami** (2006) — Action-adventure — Assessment: 9.5/10
   Watercolor art style, Zelda-style dungeons, and a paintbrush mechanic where drawing shapes affects the world. The tutorial is long. The rest of it earns the patience.

7. **Resident Evil 4** (2005) — Action-survival horror — Assessment: 10/10
   The game that redefined third-person action. Over-the-shoulder camera, context-sensitive actions, and inventory management as tension mechanic. Influenced every subsequent action game.

8. **Kingdom Hearts II** (2005) — Action-RPG — Assessment: 9/10
   Better combat than the original, weaker narrative connective tissue. The Valor/Wisdom/Master Form system adds genuine depth to the hack-and-slash base. The story assumes you played Chain of Memories.

9. **Jak II** (2003) — Action-adventure — Assessment: 8.5/10
   A harder, darker open-world follow-up to Jak and Daxter. Hovercraft, weapons, and a city that feels populated. The difficulty spikes sharply but the world design rewards exploration.

10. **Dragon Quest VIII: Journey of the Cursed King** (2004) — RPG — Assessment: 9/10
    The first mainline Dragon Quest in full 3D. Turn-based combat without abstraction, a cheerful open world, and a monster team mechanic that adds post-game depth.

---

## GameCube

The GameCube's library is small and excellent. Dolphin runs it at 4K on the UM790Pro without effort. Some GameCube games benefit significantly from the widescreen patches Dolphin supports.

1. **The Legend of Zelda: The Wind Waker** (2002) — Action-adventure — Assessment: 9.5/10
   Cell-shaded sailing adventure. Initially divisive for the art style; reappraised as one of the best Zeldas for its expression and exploration. Dolphin's widescreen patch is recommended.

2. **Metroid Prime** (2002) — First-person adventure — Assessment: 10/10
   The 3D reinvention that should not have worked. Scanning mechanic, environmental storytelling, and world design that rewards attention. Best played with the Prime Remaster controls via Dolphin patches.

3. **Resident Evil 4** (2005) — Action-survival horror — Assessment: 10/10
   The GameCube version is the original and best. Slightly better textures than the PS2 port and the version the game was designed for.

4. **Super Smash Bros. Melee** (2001) — Fighting — Assessment: 9.5/10
   The fastest and most technically demanding Smash game. The GameCube controller is still the preferred input for competitive players twenty years later. Dolphin handles it accurately.

5. **Paper Mario: The Thousand-Year Door** (2004) — RPG — Assessment: 9.5/10
   The best Paper Mario by consensus. Audience mechanic, chapter-based structure, and writing that treats side characters as real characters.

6. **F-Zero GX** (2003) — Racing — Assessment: 9.5/10
   Sega-developed F-Zero with the fastest sense of speed of any racing game. Brutal difficulty curve; the story mode is intentionally punishing. The music is louder than it needs to be, which is correct.

7. **Pikmin 2** (2004) — Strategy — Assessment: 9/10
   No time limit, two captains, and a dungeon system that adds roguelite randomness. More comfortable than the original at the cost of some tension.

8. **Eternal Darkness: Sanity's Requiem** (2002) — Survival horror — Assessment: 9/10
   Silicon Knights' only great game. Sanity system causes fake system crashes, deleted save files, and muted TVs. Twelve playable characters across three thousand years of history.

9. **Tales of Symphonia** (2003) — Action-RPG — Assessment: 9/10
   Linear Motions battle system, a dual-world structure, and a cast that expands gradually. The longest Tales game and the one that defined the Western fanbase for the series.

10. **Ikaruga** (2003) — Shoot-em-up — Assessment: 9.5/10
    The GameCube port of the Dreamcast original. Identical experience; choose whichever platform you prefer.

---

## Nintendo DS

The DS library is a goldmine of RPGs, visual novels, and experimental titles that used the dual screens and touch input in ways no other platform could replicate. DeSmuME and MelonDS both handle it on Batocera; MelonDS is more accurate.

1. **Chrono Trigger DS** (2008) — RPG — Assessment: 10/10
   The best version of the best RPG, with added content including a bonus dungeon, a new ending, and a wireless arena mode. The touch screen adds a useful map layer.

2. **Castlevania: Dawn of Sorrow** (2005) — Action-RPG — Assessment: 9.5/10
   The direct sequel to Aria of Sorrow. Soul Absorb mechanic returns, Magic Seal touch-screen finishers are the one addition that feels tacked on. Otherwise the best DS Castlevania.

3. **The World Ends With You** (2007) — Action-RPG — Assessment: 9.5/10
   Dual-screen simultaneous combat, Shibuya setting, and a story about social isolation that earns its emotional weight. Unique in ways that have not been replicated.

4. **Pokémon HeartGold/SoulSilver** (2009) — RPG — Assessment: 9.5/10
   The Johto remakes with the Pokéwalker pedometer and the entire Kanto region as post-game. The most content-complete Pokémon game on the DS.

5. **Dragon Quest IX: Sentinels of the Starry Skies** (2009) — RPG — Assessment: 9/10
   Multiplayer co-op RPG with a character creation system and a post-game grottos system that randomized dungeons. The social design is lost without local co-op, but the single-player campaign is excellent.

6. **Radiant Historia** (2010) — RPG — Assessment: 9/10
   Time-travel mechanics let you replay decisions and branch between two timelines. Grid-based combat that rewards positioning. One of the DS's most underplayed titles.

7. **999: Nine Hours, Nine Persons, Nine Doors** (2009) — Visual novel — Assessment: 9.5/10
   Escape room puzzles and a branching narrative with nine possible endings. The true ending requires specific prior runs. Disturbing, clever, and built for the DS format.

8. **Ghost Trick: Phantom Detective** (2010) — Puzzle/Adventure — Assessment: 9.5/10
   Capcom puzzle game from the Ace Attorney team. Possess objects to prevent deaths. The ending earns everything that precedes it.

9. **Fire Emblem: Shadow Dragon** (2008) — Tactical RPG — Assessment: 8/10
   Remake of the original Famicom Fire Emblem. Bare bones by Fire Emblem standards but historically significant and mechanically sound. The permadeath and touch-screen unit management work well together.

10. **Ace Attorney: Apollo Justice** (2007) — Adventure — Assessment: 8.5/10
    Fourth entry in the Ace Attorney series. New protagonist, juror system, and the series' first move toward HD assets. Best played after the Phoenix Wright trilogy.

---

## PSP

The PSP punched well above its handheld weight class. Its library skews toward action games and RPGs that suited the format. PPSSPP on Batocera is essentially perfect, with full upscaling.

1. **God of War: Ghost of Sparta** (2010) — Action — Assessment: 9.5/10
   Better than Chains of Olympus and on par with the PS2 entries. The PSP's strongest technical showcase and the deepest narrative of the portable God of War games.

2. **Crisis Core: Final Fantasy VII** (2007) — Action-RPG — Assessment: 9/10
   Zack Fair's story. Digital Mind Wave slot mechanic adds randomness to combat; the conclusion is one of the most effective scenes in the series. Requires no prior FF7 knowledge but adds to it.

3. **Persona 3 Portable** (2009) — RPG — Assessment: 9.5/10
   The Persona 3 experience compressed for handheld play. Female protagonist route adds substantial new social links and a different perspective on the story. No animated cutscenes in this version.

4. **Monster Hunter Freedom Unite** (2008) — Action-RPG — Assessment: 9/10
   The game that built the Monster Hunter audience outside Japan. Hundreds of hours of cooperative hunting, thousands of equipment combinations. Best with local co-op; solo is demanding.

5. **Tactics Ogre: Let Us Cling Together** (2010) — Tactical RPG — Assessment: 9.5/10
   The definitive version of the SNES classic. Chariot system lets you rewind battles to change decisions; Ogre series CODA epilogue adds post-game depth. One of the finest tactical RPGs made.

6. **Final Fantasy Tactics: War of the Lions** (2007) — Tactical RPG — Assessment: 9.5/10
   The PSP port of FFT with new cutscenes, new jobs, and a revised translation. The deepest tactics game on the platform and one of the deepest anywhere.

7. **Lumines** (2004) — Puzzle — Assessment: 9/10
   Tetris-adjacent block dropping with a musical skin system that changes the visual and audio layer as you progress. Hypnotic and depth-concealing.

8. **Jeanne d'Arc** (2006) — Tactical RPG — Assessment: 8.5/10
   Level-5's tactical RPG based on Joan of Arc with a fantasy redesign. Burn system, bangle mechanics, and a surprisingly well-paced campaign. Underplayed.

9. **Valkyria Chronicles II** (2010) — Tactical RPG — Assessment: 8.5/10
   Smaller scale than the PS3 original but mechanically refined. Class advancement system and mission-based structure suit the portable format.

10. **Grand Theft Auto: Vice City Stories** (2006) — Action-adventure — Assessment: 8.5/10
    The better of the two PSP GTA titles. Larger than Liberty City Stories, the 1980s Vice City setting suits the format, and the empire-building mechanic adds a strategic layer.

---

## Wii

The Wii's motion controls age unevenly, but its library includes some of the best games of the generation and several that require no motion input at all. Dolphin handles it all, and motion can be emulated via controller if you don't have a sensor bar.

1. **Super Mario Galaxy** (2007) — 3D platformer — Assessment: 10/10
   Gravity-defying spherical worlds and the series' most creative level design since Mario 64. The co-star pointer mechanic is optional. One of the clearest cases for Nintendo's design philosophy.

2. **The Legend of Zelda: Twilight Princess** (2006) — Action-adventure — Assessment: 9/10
   The more conventional Zelda after Wind Waker's experiments. Darker tone, horseback combat, and the best dungeon design of any 3D Zelda. Motion controls are functional; the GameCube version is available via Dolphin if preferred.

3. **Metroid Prime 3: Corruption** (2007) — First-person adventure — Assessment: 9/10
   The motion control implementation that worked. Pointer aiming is genuinely superior to analog sticks for this type of game. The weakest Prime narrative but strong level design throughout.

4. **Super Smash Bros. Brawl** (2008) — Fighting — Assessment: 9/10
   Largest roster of the trilogy, slowest movement, Subspace Emissary campaign. The online mode is dead; local play is unaffected.

5. **Mario Kart Wii** (2008) — Racing — Assessment: 9/10
   The motion-controlled entry with the largest online player base in series history. Twelve-player battles, unlockable vehicles, and courses that remain in the series today.

6. **Xenoblade Chronicles** (2010) — RPG — Assessment: 9.5/10
   Large-scale open-world RPG with real-time combat. The Monado foresight mechanic and interconnected world make it one of the most ambitious RPGs of the generation. Long.

7. **No More Heroes** (2007) — Action — Assessment: 8.5/10
   Grasshopper Manufacture's stylized assassination tournament. The beam katana combat is better than it looks and the boss encounters are memorable set pieces. The open world is sparse and mostly not worth engaging.

8. **Donkey Kong Country Returns** (2010) — Platformer — Assessment: 9/10
   Retro Studios' revival of the DKC series. Precision platforming, silhouette levels, and a two-player co-op mode. The MINE CART levels will be remembered.

9. **Punch-Out!!** (2009) — Sports — Assessment: 9/10
   The Wii revival that nailed the pattern-recognition gameplay of the NES original while adding motion control uppercuts that actually work. Little Mac's opponents are as memorable as ever.

10. **The Last Story** (2011) — RPG — Assessment: 8.5/10
    Hironobu Sakaguchi's RPG outside Square Enix. Real-time combat with cover mechanics, a compact world, and a story that earns its ending. One of three notable JRPGs released in the Wii's final years alongside Xenoblade and Pandora's Tower.

---

## PlayStation 3

RPCS3 on the UM790Pro runs most of the PS3 library well via Vulkan. Performance varies by title; the games below are confirmed playable at good frame rates on hardware of this class. Check [rpcs3.net/compatibility](https://rpcs3.net/compatibility) before downloading a specific title. PS3 firmware (`PS3UPDAT.PUP`) is required and must be installed through RPCS3's settings panel.

1. **The Last of Us** (2013) — Action-adventure — Assessment: 10/10
   RPCS3 status: Playable. Naughty Dog's survival story. The crafting system, AI companion design, and narrative integration are the benchmark the genre has chased since. Plays well on RPCS3 at native resolution.

2. **Demon's Souls** (2009) — Action-RPG — Assessment: 9.5/10
   RPCS3 status: Playable. The original Souls game. World-tendency system, interconnected world, and boss design that FromSoftware refined in later games. The PS5 remake exists but changes the tone; the original is darker.

3. **Uncharted 2: Among Thieves** (2009) — Action-adventure — Assessment: 9.5/10
   RPCS3 status: Playable. The game that established the cinematic action-adventure genre conventions. Train sequence, Nepal chapter, and co-op mode. Plays well on RPCS3.

4. **Metal Gear Solid 4: Guns of the Patriots** (2008) — Stealth-action — Assessment: 9/10
   RPCS3 status: Playable. The conclusion of the Snake saga. More cutscene than game by ratio, but the Act 4 callback sequence alone earns the playthrough. Long install times in the original are irrelevant under emulation.

5. **God of War III** (2010) — Action — Assessment: 9.5/10
   RPCS3 status: Playable. The PS3's technical showpiece. The Poseidon fight opening is the most impressive first ten minutes of any game in the series.

6. **Red Dead Redemption** (2010) — Open world — Assessment: 9.5/10
   RPCS3 status: Playable (significant improvements recent). The Western that established Rockstar's open-world template outside the GTA framework. Undead Nightmare DLC is included in some versions.

7. **BlazBlue: Calamity Trigger** (2008) — Fighting — Assessment: 8.5/10
   RPCS3 status: Playable. The first in Arc System Works' anime fighter series. Deeper than it appears, with an Arcade Story mode that contextualizes the roster. The series' lore accumulates across entries.

8. **Persona 5** (2016) — RPG — Assessment: 9.5/10
   RPCS3 status: Playable. Available on PS3 and PS4; the PS3 version runs under RPCS3 with some performance concessions compared to PS4. Style, Palaces, social simulation, and turn-based combat that rewards status effects. The Royal version adds content but requires PS4 emulation.

9. **3D Dot Game Heroes** (2009) — Action-adventure — Assessment: 8.5/10
   RPCS3 status: Playable. Voxel-art Zelda tribute with a character editor and enormous sword upgrades. Deliberate homage rather than parody.

10. **Valkyria Chronicles** (2008) — Tactical RPG — Assessment: 9.5/10
    RPCS3 status: Playable. Also available on PC via Steam, which is the simpler route. Watercolor aesthetic, BLITZ turn-based system, and a World War II analogue story. The PS3 version runs cleanly under RPCS3.

---

## Xbox 360

Xenia's compatibility is incomplete and varies significantly by title. The list below reflects games with known-good or mostly-playable status in Xenia as of 2026. Check compatibility before investing time in a download. Many Xbox 360 titles are available on PC via backward compatibility or Steam and are easier to run that way.

1. **Red Dead Redemption** (2010) — Open world — Assessment: 9.5/10
   Xenia status: Playable with minor issues. See the PS3 entry above. The 360 version is a viable route if RPCS3 proves difficult for this title.

2. **Halo 3** (2007) — First-person shooter — Assessment: 9/10
   Xenia status: Ingame (some graphical issues). The campaign conclusion of the original Bungie trilogy. Four-player co-op, Forge mode, and a multiplayer that defined the generation. The Master Chief Collection on PC is the cleaner option.

3. **Gears of War** (2006) — Third-person shooter — Assessment: 9/10
   Xenia status: Playable. Cover-based third-person shooting that defined the genre for a decade. Chainsaw bayonet. Two-player co-op campaign.

4. **Mass Effect** (2007) — Action-RPG — Assessment: 9.5/10
   Xenia status: Playable with some issues. The PC version through EA App is more reliable. The original Mass Effect with the planetary exploration and conversation wheel that the sequels simplified.

5. **BioShock** (2007) — First-person shooter — Assessment: 9.5/10
   Xenia status: Ingame. Rapture, plasmids, and "Would you kindly." The PC remastered version is the simpler option. Xenia can get this running but with effort.

6. **Fable II** (2008) — Action-RPG — Assessment: 8.5/10
   Xenia status: Playable. The Lionhead RPG with consequence-driven design and a dog companion that reacts to player choices. Never released on PC, making Xenia the only modern option.

7. **The Elder Scrolls IV: Oblivion** (2006) — Open-world RPG — Assessment: 8.5/10
   Xenia status: Playable. The PC version is broadly superior, but Xenia handles it. The radiant AI system and Oblivion gates are more interesting than the game's reputation suggests.

8. **Left 4 Dead 2** (2009) — Co-op shooter — Assessment: 9/10
   Xenia status: Variable. Four-player co-op zombie survival with a director AI that adjusts difficulty based on player performance. The PC version via Steam is vastly preferable.

9. **Banjo-Kazooie: Nuts and Bolts** (2008) — Vehicle action — Assessment: 7.5/10
   Xenia status: Playable. Divisive as a Banjo-Kazooie entry because it replaced platforming with vehicle construction. On its own terms as a vehicle builder, it is excellent. Never released on PC, making Xenia the only modern route.

10. **Trials HD** (2009) — Physics platformer — Assessment: 8.5/10
    Xenia status: Playable. XBLA-exclusive motorcycle physics platformer. PC successors are available via Steam (Trials Fusion, Rising) but the original has a purity the sequels added features to.

---

## 3DS

The 3DS library is enormous. Batocera runs it via Lime3DS (a Citra fork). Performance on the UM790Pro handles everything except the most demanding titles without configuration. 3D effect is disabled in emulation; it is not missed.

1. **Fire Emblem: Awakening** (2012) — Tactical RPG — Assessment: 9.5/10
   The game that saved the Fire Emblem series commercially. Marriage system, relationship support mechanics, and a roster large enough to accommodate multiple playstyle approaches. Pair-Up combat redefined the series.

2. **The Legend of Zelda: A Link Between Worlds** (2013) — Action-adventure — Assessment: 9.5/10
   A Link to the Past successor with a wall-merging mechanic. Item rental system removes the dungeon-order constraint; the world design rewards experimentation.

3. **Pokémon X/Y** (2013) — RPG — Assessment: 8.5/10
   The first mainline Pokémon in full 3D. Kalos region, Fairy type, and Mega Evolution. The post-game is thin; the main campaign is the point.

4. **Monster Hunter 4 Ultimate** (2014) — Action-RPG — Assessment: 9.5/10
   The best Monster Hunter before World streamlined the formula. Mounting system, a large roster of weapons, and online co-op for up to four players. Long-term game.

5. **Shin Megami Tensei IV** (2013) — RPG — Assessment: 9/10
   Press Turn combat, demon fusion, and a post-apocalyptic Tokyo. Harder than Persona and deliberately uncompromising. Multiple endings with a neutral path that requires effort to reach.

6. **Bravely Default** (2012) — RPG — Assessment: 9/10
   Job system RPG with a Brave/Default mechanic that lets you bank or spend actions across turns. The structure's late-game repetition is a deliberate narrative choice; it works better than it should.

7. **Kid Icarus: Uprising** (2012) — Action — Assessment: 9/10
   Third-person rail shooter that transitions to arena combat, written by Masahiro Sakurai with dialogue that anticipates and responds to player behavior. Control scheme requires the 3DS cradle; emulation removes this constraint.

8. **Luigi's Mansion: Dark Moon** (2013) — Action-adventure — Assessment: 8.5/10
   Ghost-hunting sequel split into discrete mission structures rather than one open mansion. Less atmospheric than the original for this reason, but mechanically richer.

9. **Xenoblade Chronicles 3D** (2015) — RPG — Assessment: 9/10
   The New 3DS port of the Wii original. Requires a New 3DS for full performance; under emulation performance is consistent. The Wii or Switch versions are preferable if available.

10. **Theatrhythm Final Fantasy: Curtain Call** (2014) — Rhythm — Assessment: 9/10
    221 songs, two gameplay modes, and a colosseum battle system. The most content-complete rhythm game on the platform. Requires no prior Final Fantasy knowledge.

---

## Wii U

The Wii U was a commercial failure with a genuinely excellent first-party library. Cemu on Batocera handles it well. The UM790Pro runs the full library at 4K/60 with Vulkan. Online services are dead; single-player and local multiplayer are unaffected.

1. **The Legend of Zelda: Breath of the Wild** (2017) — Action-adventure — Assessment: 10/10
   The open-world reinvention of Zelda. Physics engine, cooking system, weapon durability, and a world that rewards curiosity without waypoints. Cemu is the best way to play this on PC hardware.

2. **Super Mario 3D World** (2013) — 3D platformer — Assessment: 9.5/10
   Four-player simultaneous 3D platformer with a Captain Toad stage in every world. The multiplayer chaos is the point. Individual stages are the most tightly designed Nintendo produced this generation.

3. **Mario Kart 8** (2014) — Racing — Assessment: 9.5/10
   Anti-gravity tracks, Blue Falcon returning, and 32 courses plus DLC that brought the total to 48. Cemu handles it at 4K/60. Online is dead; local eight-player works via LAN.

4. **Bayonetta 2** (2014) — Action — Assessment: 9.5/10
   PlatinumGames' best work and a Wii U exclusive. Witch Time, combo depth, and a difficulty system that rewards engagement at every level. Includes the original Bayonetta.

5. **Pikmin 3** (2013) — Strategy — Assessment: 9/10
   Three captains, fruit-collection objectives, and a brutally efficient puzzle-strategy loop. Mission Mode and Bingo Battle add replay value beyond the campaign.

6. **Donkey Kong Country: Tropical Freeze** (2014) — Platformer — Assessment: 9.5/10
   Retro Studios' second DKC entry. David Wise returns for the soundtrack, the difficulty is high, and the level design is among the finest in the genre.

7. **Splatoon** (2015) — Shooter — Assessment: 9/10
   The first Splatoon. Ink-coverage objective mode, tight movement, and a visual identity that the series has maintained. Online servers are down; the single-player campaign and local modes are intact.

8. **Super Smash Bros. for Wii U** (2014) — Fighting — Assessment: 9/10
   Larger roster than Brawl, faster movement, and better competitive balance than its predecessor. The 8-player smash mode fills a room.

9. **Xenoblade Chronicles X** (2015) — RPG — Assessment: 8.5/10
   Open-world RPG with a Skell mech system that opens up when the main story is roughly half done. More mechanically complex than Xenoblade Chronicles with a weaker narrative. Long.

10. **Star Fox Zero** (2016) — Rail shooter — Assessment: 7.5/10
    Divided reception due to dual-screen required controls. Under Cemu with a single screen and remapped controls, the control complaint largely disappears. Mechanically closer to the original Star Fox 64 than anything since.

---

## Arcade (MAME / FinalBurn Neo)

The arcade library in Batocera via MAME and FinalBurn Neo covers tens of thousands of titles across six decades — narrowing it to ten is an act of violence. These are ten all-time standouts that represent the range of what the medium produced, all running accurately in Batocera. MAME prioritises accuracy; FinalBurn Neo prioritises compatibility for fighters and shmups. For the titles below, either core works.

1. **Street Fighter II: The World Warrior** (1991) — Fighting — Assessment: 10/10
   The game that created competitive gaming as a genre. Six buttons, eight characters, and a combo system discovered by players rather than designed by Capcom. The subsequent revisions improved it; this is the original statement.

2. **Pac-Man** (1980) — Maze — Assessment: 10/10
   The most recognised video game ever made. The pattern-based ghost AI, the power pellets, and the maze design hold up as a pure test of spatial awareness and timing. The arcade original is better than any home port.

3. **Galaga** (1981) — Shoot-em-up — Assessment: 9.5/10
   The refinement of Space Invaders. The tractor beam capture mechanic and dual-fighter mode add strategy to a genre that was previously pure reflex. Still holds up as a score-chasing game.

4. **Donkey Kong** (1981) — Platformer — Assessment: 9.5/10
   The game that introduced Mario (as Jumpman) and proved that arcade games could tell a story. Four stages, a rolling barrel, and a vertical structure that nobody had tried before. The original of the genre.

5. **Contra** (1987) — Run-and-gun — Assessment: 9.5/10
   The arcade original is harder and longer than the NES port. Two-player simultaneous, five weapon types, and a spread gun that made every player feel like they were in an action movie. The template.

6. **1942** (1984) — Shoot-em-up — Assessment: 8.5/10
   Capcom's vertical shmup set in the Pacific War. The loop-the-loop dodge mechanic and the waves of formation enemies established patterns the genre built on for a decade. Foundational.

7. **Bubble Bobble** (1986) — Platformer — Assessment: 9/10
   Trap enemies in bubbles, pop them for fruit, collect everything before the timer runs out. Two-player cooperative play, 100 stages, and a true ending that requires both players to reach it together. One of the most replayable arcade games ever designed.

8. **OutRun** (1986) — Racing — Assessment: 9/10
   Yu Suzuki's masterpiece. A branching road system, a Ferrari Testarossa, and a soundtrack you choose at the start. The sense of speed in 1986 was unmatched. Still one of the best pure driving experiences in gaming.

9. **Mortal Kombat II** (1993) — Fighting — Assessment: 9/10
   The refinement of the original's formula. Better balance, more moves per character, and the fatalities that made the series infamous. The peak of the series' classic era and the version that defines it in competitive memory.

10. **R-Type** (1987) — Shoot-em-up — Assessment: 9.5/10
    Irem's horizontal shooter introduced the Force pod — an indestructible option that attaches to your ship — and a design philosophy where memorisation is the skill being tested, not reaction time. One of the most influential shooters ever made and still one of the most demanding.

---

## How much to trust this

Assessment throughout. Scores reflect critical consensus at the time of the game's peak reputation — a combination of review aggregates, longterm community regard, and competitive scene durability where relevant. No score here reflects play-testing on this specific hardware configuration.

For PS3 and Xbox 360 specifically: emulator compatibility changes with every release. Before committing to a download, check the official compatibility databases:
- RPCS3: [rpcs3.net/compatibility](https://rpcs3.net/compatibility)
- Xenia: [xenia.jp](https://xenia.jp) (or the GitHub issues tracker for specific titles)

For everything else: mGBA, Flycast, PCSX2, Dolphin, and Cemu are mature emulators with broad compatibility. The games on this list run without significant issue on the UM790Pro hardware class.

*This list was compiled 2026-08-31 for the [Batocera UM790Pro build](emulation-station-batocera).*
